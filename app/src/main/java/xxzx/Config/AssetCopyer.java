...
public class AssetCopyer {
    ...
    public boolean copy() throws IOException {
        ...
        //该目录存放app相关的各种文件(如cache，配置文件等)
        //创建system根目录
        configDirectory = new File(MyString.config_folder_path);
        ...
        for( String asset : assets ) {
            //如果不存在，则添加到copy列表
            if( ! new File(configDirectory,asset).exists() ) {
                srcFiles.add(asset);
            }
        }
        //依次拷贝到App的安装目录下
    ...
    protected File copy( String asset ) throws IOException {
        ...
        File destinationFile = new File(configDirectory, asset);
